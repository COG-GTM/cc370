...
|----|----|----|
| Decimal precision and rounding | Implemented | Integer half-up in both implementations; `test_decimal_against_independent_rational_math` re-derives interest and charge with `Fraction`, touching neither `Decimal` nor the oracle's rounder |
...
| Restart and partial failure | Implemented at the oracle and record level | Replay stages return all-`DUPL`; a split mid-stream then resumed reproduces the uninterrupted master byte for byte. **The program itself has no checkpoint record**: an interrupted run is discarded and rerun from the input generation |
| Duplicate and conflict | Implemented | `DUPL`/`CNFL`/`ORDR` with 244+ cases per stage |
...
| Execution evidence | Deferred | No golden file is an observed MVS result; `tools/compare.py` refuses to pass without a run receipt carrying a job id, RC 0, no ABEND, no timeout, and matching input/output hashes |
