...
## 8. Independent parent verification (`194aedf7`)

The parent session independently verified the implementation commit
`194aedf7` on its own machine and retained its own evidence, separate from
and not relabelling anything in this directory:

- review: <https://app.devin.ai/attachments/4b9cf03f-4aef-4290-834e-6477350d519d/recovery-acceptance-review.md>
- complete evidence bundle:
  <https://app.devin.ai/attachments/05be768a-02e7-4fc9-86ca-c9f367b1aa72/parent-recovery-194aedf.tar.gz>
  SHA-256 `b7bcc40b632247889aad979aa5483068bb8ad9c6001f7791fd63de65195429fe`
- bound to source `194aedf7` and the parent's own packaged JAR
  `327f3cd01ec8fd02aa2b3f07c7b8f2162c05e4bba6b31f675936fc4804b006b0`
  (an independent build, so it differs from the JAR hashes in this
  directory by design)
- scope as reported by the parent: clean `./mvnw verify` 156 tests
  (7 + 21 + 40 + 88); all batch / raw HTTP / hybrid HTTP corpus stages and
  targeted cases (24 reports); routines 184 × 3; the 33 authority controls and
  the response-mutation negatives; and an actual packaged-JAR (`java -jar`)
  recovery against PostgreSQL — crash after 2,049 committed entries, claim at
  fence 2, a repeated same-owner HTTP claim returning the identical
  fence/claim count with a single claim-history row, resume at 2,050, final
  4,097 requests/results/master byte-exact to the seven-hash-validated
  retained A2 authority.
- no new guest execution: the frozen guest authorities were replayed
  unchanged, exactly as for the child evidence here.

The child evidence in this directory keeps its own labels (`cf9aa9b0` / JAR
`2a4615b2…` for the parity reports, `9e86b78f` for `runtime/`, JAR
`91978b63…` for the child build) — the two sets are separate observations of
the same source, not one relabelled as the other.
...
## 10. Retained runtime evidence (`runtime/`)
